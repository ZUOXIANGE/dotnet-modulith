...
import { useMessage, useDialog, type FormInst, type FormRules, type DataTableColumns, NButton, NTag, type SelectOption } from 'naive-ui'
...
const statusTagMap: Record<string, { type: 'info' | 'success' | 'warning' | 'error'; label: string }> = {
  ...
  Fulfilled: { type: 'success', label: '已满足' },
...
const columns: DataTableColumns<ReservationItem> = [
  ...
  { title: '到期日期', key: 'expiryDate', width: 110 },
  { title: '队列位置', key: 'queuePosition', width: 80 },
  {
    title: '状态',
    key: 'status',
    width: 90,
...
async function handleCancel(row: ReservationItem) {
  dialog.warning({
    title: '确认取消',
    content: `确定取消对《${row.bookTitle}》的预约吗？`,
    positiveText: '确认',
    negativeText: '取消',
